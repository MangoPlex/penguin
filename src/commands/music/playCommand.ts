...
export class PlayCommand extends Command {
  ...
  public async play(interaction: Command.ChatInputInteraction): Promise<void> {
    ...
    if (!memState?.channel) {
      ...
      if (!botState.channel || !player)
        player = lavalink?.createPlayer(interaction.guildId!);
    ...
    if (this.checkUrl(url)) res = await lavalink?.rest?.loadTracks(url);
    ...
    switch (res?.loadType) {
      ...
      case "PLAYLIST_LOADED":
        ...
        await interaction.editReply({
          embeds: [
            new MessageEmbed().setDescription(
              `Added playlist **${res.playlistInfo.name}** to queue`
            ),
          ],
        });
        break;
      case "NO_MATCHES":
        await interaction.editReply({
          embeds: [new MessageEmbed().setDescription(`Not found`)],
        });
        break;
    }
    ...
    if (!player?.trackData) await player?.queue.start();
  }
    ...
    registry.registerChatInputCommand((builder) =>
      builder
        ...
        .addStringOption((input) =>
          input.setName("url").setDescription("Url/keyword of the track")
        )
    );
  }
}
